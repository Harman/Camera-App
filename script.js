...
recordBtn.addEventListener("click", function (e) {
  if (isRecording) {
    ...
    mediaRecorder.start();
    appliedFilter = ""; //color removed as video don't have filters
    removeFilter(); //removed from UI
    currZoom = 1;
    video.style.transform = `scale(${currZoom})`;
    isRecording = true;
...
capBtn.addEventListener("click", function () {
  ...
  //for image we will save this data url in IndexdedDB
...
navigator.mediaDevices
  ...
  .then(function (mediaStream) {
    ...
    mediaRecorder.addEventListener("stop", function (e) {
      //for video we will save this blob object in IndexedDB
